...
void OpeDB::init()
{
    //连接服务器，如果连接的时本地服务器就localhost，如果是远程则该为地IP
...
void OpeDB::handleArgeeAddFriend(const char *pername, const char *name)
{
...
bool OpeDB::handleDelFriend(const char *name, const char *friendName)
{
    ...
    QString data = QString("delete from friend where id in(select id from usrInfo where name=\'%1\') and friendId=(select id from usrInfo where name=\'%2\')").arg(name).arg(friendName);
    QSqlQuery query;
    query.exec(data);

    data = QString("delete from friend where id in(select id from usrInfo where name=\'%1\') and friendId=(select id from usrInfo where name=\'%2\')").arg(friendName).arg(name);
    query.exec(data);
